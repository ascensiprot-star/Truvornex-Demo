...
function DemandBar({ demand_index, category, price }) {
    return (
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
            ...
            <span style={{ width: 90, fontSize: 14, color: 'var(--color-text)', textTransform: 'capitalize', flexShrink: 0 }}>{category}</span>
            ...
            <span style={{ fontSize: 14, color: DEMAND_COLOR(demand_index), fontWeight: 600, width: 34, textAlign: 'right' }}>{demand_index}</span>
            <span style={{ fontSize: 13, color: 'var(--color-text-subtle)', width: 80, textAlign: 'right' }}>₨{price?.toLocaleString()}</span>
        </div>
    );
}
...
function SectionHead({ title, sub }) {
    return (
        <div style={{ marginBottom: 16 }}>
            ...
            {sub && <p style={{ color: 'var(--color-text-subtle)', fontSize: 16, margin: '4px 0 0' }}>{sub}</p>}
        </div>
    );
}
    ...
    return (
        <div style={{ maxWidth: 780, margin: '0 auto', padding: '28px 20px 80px' }}>
            <div style={{ marginBottom: 24 }}>
                <div style={{ fontSize: 13, color: 'var(--color-text-subtle)', letterSpacing: 2, textTransform: 'uppercase', marginBottom: 6 }}>
                ...
                <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                    <input value={area} onChange={e => setArea(e.target.value)} placeholder="Area name"
                        style={{ background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 8, padding: '8px 14px', color: 'var(--color-text)', fontSize: 16, width: 180 }} />
                    ...
                    <button onClick={() => { loadForecast(); loadSimonForecast(); }} style={{
                        ...
                        borderRadius: 8, padding: '8px 18px', fontSize: 16, fontWeight: 600, cursor: 'pointer',
            ...
            {topOpp && (
                <div style={{
                    ...
                    <div>
                        <div style={{ fontSize: 16, fontWeight: 700, color: 'var(--color-text)', marginBottom: 2 }}>
                            Top Opportunity — {topOpp.category ? topOpp.category.charAt(0).toUpperCase() + topOpp.category.slice(1) : ''}
                        </div>
                        <div style={{ fontSize: 16, color: 'var(--color-text-subtle)' }}>{topOpp.reason}</div>
                        <div style={{ fontSize: 13, color: 'var(--color-text-subtle)', marginTop: 4 }}>Living wage floor: ₨800/hr guaranteed minimum</div>
                    </div>
                </div>
            )}
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 20, marginBottom: 24 }}>
                <Card>
                    ...
                    {topForecast.length === 0 ? (
                        <p style={{ color: 'var(--color-text-subtle)', fontSize: 16 }}>Loading Simon forecast...</p>
                    ) : (
                        topForecast.map(f => (
                            <div key={f.category} style={{ marginBottom: 10 }}>
                                ...
                                {f.supply_shortfall && (
                                    <div style={{ fontSize: 12, color: '#ef4444', marginLeft: 120, marginTop: -4, marginBottom: 4 }}>
                                        ⚠ Supply shortfall — providers urgently needed
                                    </div>
                                )}
                                {f.opportunity_note && (
                                    <div style={{ fontSize: 12, color: 'var(--color-text-subtle)', marginLeft: 120, marginTop: -4, marginBottom: 4 }}>
                                        {f.opportunity_note}
                                    </div>
                                )}
                            </div>
                ...
                <Card>
                    ...
                    {forecastLoading ? (
                        <p style={{ color: 'var(--color-text-subtle)', fontSize: 16 }}>Generating forecast...</p>
                        ...
                        <p style={{ color: 'var(--color-text-subtle)', fontSize: 16 }}>No forecast data yet.</p>
                    )}
                ...
                <Card style={{ marginBottom: 24 }}>
                    ...
                    {microJobs.slice(0, 6).map(job => (
                        <div key={job.id} style={{
                            ...
                            fontSize: 16, color: 'var(--color-text)', gap: 12,
                        }}>
                            <span style={{ fontWeight: 600 }}>{CAT_ICONS[job.category] || '⚙️'} {job.title}</span>
                            <span style={{ color: 'var(--color-text-subtle)', fontSize: 14 }}>{job.estimated_duration_hours}h</span>
                ...
                <>
                    <Card style={{ marginBottom: 20 }}>
                        ...
                        <form onSubmit={submitIdleSlot} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                                <div>
                                    <label style={{ fontSize: 14, color: 'var(--color-text-subtle)', display: 'block', marginBottom: 4 }}>Window Start</label>
                                    <input type="datetime-local" value={idleStart} onChange={e => setIdleStart(e.target.value)} required
                                        style={{ width: '100%', background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 8, padding: '9px 12px', color: 'var(--color-text)', fontSize: 16 }} />
                                </div>
                                <div>
                                    <label style={{ fontSize: 14, color: 'var(--color-text-subtle)', display: 'block', marginBottom: 4 }}>Window End</label>
                                    <input type="datetime-local" value={idleEnd} onChange={e => setIdleEnd(e.target.value)} required
                                        style={{ width: '100%', background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 8, padding: '9px 12px', color: 'var(--color-text)', fontSize: 16 }} />
                                </div>
                            </div>
                            <div>
                                <label style={{ fontSize: 14, color: 'var(--color-text-subtle)', display: 'block', marginBottom: 6 }}>Your Skills (optional — leave blank for all)</label>
                            ...
                            <button type="submit" disabled={idleLoading} style={{
                                background: 'var(--color-primary)', color: '#fff', border: 'none',
                                borderRadius: 8, padding: '10px', fontSize: 16, fontWeight: 600, cursor: 'pointer', alignSelf: 'flex-start',
                            }}>
                                {idleLoading ? 'Registering...' : 'Register Window'}
                            </button>
                            {idleMsg && <p style={{ fontSize: 16, color: 'var(--color-primary)', margin: 0 }}>{idleMsg}</p>}
                        </form>

                        {idleSlots.length > 0 && (
                            <div style={{ marginTop: 16, borderTop: '1px solid var(--color-border)', paddingTop: 14 }}>
                                <div style={{ fontSize: 14, color: 'var(--color-text-subtle)', marginBottom: 8 }}>Your Idle Windows</div>
                                {idleSlots.map(s => (
                                    <div key={s.id} style={{
                                        display: 'flex', alignItems: 'center', justifyContent: 'space-between',
                    ...
                    <Card style={{ marginBottom: 20 }}>
                        ...
                        <button onClick={runIdleMatch} disabled={matchLoading} style={{
                            background: 'var(--color-primary)', color: '#fff', border: 'none',
                            borderRadius: 8, padding: '10px 20px', fontSize: 16, fontWeight: 600, cursor: 'pointer', marginBottom: 16,
                        }}>
                            {matchLoading ? 'Simon is matching…' : '⚡ Find Micro-Jobs for My Windows'}
                        </button>
                        {matches && (
                            <div>
                                <p style={{ fontSize: 16, color: 'var(--color-text-subtle)', marginBottom: 12 }}>{matches.message}</p>
                                {matches.matches?.map(job => (
                                    <div key={job.id} style={{
                                        ...
                                        <div style={{ flex: 1 }}>
                                            ...
                                            <div style={{ fontSize: 14, color: 'var(--color-text-subtle)', marginBottom: 4 }}>{job.description}</div>
                                        ...
                                        <div style={{ textAlign: 'right', flexShrink: 0 }}>
                                            ...
                                            <button onClick={() => acceptMicroJob(job.id)} style={{
                                                ...
                                                borderRadius: 6, padding: '5px 12px', fontSize: 14, fontWeight: 600, cursor: 'pointer', marginTop: 6,
                                            }}>Accept</button>
                                        </div>
                                    </div>
                                ))}
                    ...
                    <Card>
                        ...
                        <form onSubmit={addGoal} style={{ display: 'flex', gap: 10, flexWrap: 'wrap', marginBottom: 16 }}>
                            <input value={goalTitle} onChange={e => setGoalTitle(e.target.value)} placeholder="Goal (e.g. Buy a generator)" required
                                style={{ flex: 1, minWidth: 160, background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 8, padding: '9px 12px', color: 'var(--color-text)', fontSize: 16 }} />
                            <input type="number" value={goalTarget} onChange={e => setGoalTarget(e.target.value)} placeholder="Target ₨" required
                                style={{ width: 120, background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 8, padding: '9px 12px', color: 'var(--color-text)', fontSize: 16 }} />
                            <input type="date" value={goalDeadline} onChange={e => setGoalDeadline(e.target.value)}
                                style={{ width: 140, background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 8, padding: '9px 12px', color: 'var(--color-text)', fontSize: 16 }} />
                            <button type="submit" style={{
                                background: 'var(--color-primary)', color: '#fff', border: 'none',
                                borderRadius: 8, padding: '9px 16px', fontSize: 16, fontWeight: 600, cursor: 'pointer',
                            }}>Add Goal</button>
                        </form>
                        {goalMsg && <p style={{ fontSize: 16, color: 'var(--color-primary)', marginBottom: 12 }}>{goalMsg}</p>}
                        {goals.map(g => {
                            ...
                            return (
                                <div key={g.id} style={{ marginBottom: 14, background: 'var(--color-bg)', borderRadius: 10, padding: 14, border: '1px solid var(--color-border)' }}>
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 6 }}>
                                        ...
                                        <span style={{ fontSize: 14, color: 'var(--color-text-subtle)' }}>₨{Number(g.saved_pkr).toLocaleString()} / ₨{Number(g.target_pkr).toLocaleString()}</span>
                                    </div>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                                        <span style={{ fontSize: 13, color: 'var(--color-text-subtle)' }}>{pct}% saved{g.simon_routing_active ? ' · Simon routing active' : ''}</span>
                                        {g.deadline && <span style={{ fontSize: 13, color: 'var(--color-text-subtle)' }}>By {g.deadline}</span>}
                                    </div>
                                </div>
                            );
                        })}
                        {goals.length === 0 && <p style={{ color: 'var(--color-text-subtle)', fontSize: 16 }}>No savings goals yet. Simon will route matching micro-jobs toward your target automatically.</p>}
                    </Card>
                </>
            )}
