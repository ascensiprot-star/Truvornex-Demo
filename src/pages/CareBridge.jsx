...
export default function CareBridge() {
    ...
    const inputStyle = {
        ...
        borderRadius: 8, padding: '10px 12px', color: 'var(--color-text)', fontSize: 18,
    };
    const labelStyle = { fontSize: 16, color: 'var(--color-text-subtle)', display: 'block', marginBottom: 4 };
            ...
            <div style={{ display: 'flex', gap: 4, background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 10, padding: 4, marginBottom: 24, width: 'fit-content' }}>
                {[['new-order', 'Send Care'], ['orders', 'My Orders']].map(([k, l]) => (
                    <button key={k} onClick={() => setTab(k)} style={{
                        ...
                        border: 'none', borderRadius: 7, padding: '8px 18px', fontSize: 18, fontWeight: 600, cursor: 'pointer',
                    }}>{l}</button>
                ))}
            </div>
                ...
                <Card>
                    ...
                    <form onSubmit={submitOrder} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <div style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 10, padding: 16 }}>
                            <div style={{ fontSize: 18, fontWeight: 600, color: 'var(--color-text)', marginBottom: 12 }}>Amount to Send</div>
                                ...
                                {SERVICE_CATS.map(({ key, icon, label }) => {
                                    ...
                                    return (
                                        <button key={key} type="button" onClick={() => toggleService(key)} style={{
                                            ...
                                            borderRadius: 8, padding: '10px', fontSize: 18, cursor: 'pointer', textAlign: 'left',
                                        }}>
                        ...
                        <Card>
                            <div style={{ textAlign: 'center', padding: '30px 0' }}>
                                ...
                                <button onClick={() => setTab('new-order')} style={{
                                    background: 'var(--color-primary)', color: '#fff', border: 'none',
                                    borderRadius: 8, padding: '10px 20px', fontSize: 18, fontWeight: 600, cursor: 'pointer', marginTop: 12,
                            ...
                            <Card key={order.id} style={{ marginBottom: 14 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                    <div>
                                        ...
                                        <div style={{ fontSize: 16, color: 'var(--color-text-subtle)' }}>{order.recipient_city}, {order.recipient_country} · {order.recipient_phone}</div>
                                    </div>
                                    <div style={{ background: sm.bg, color: sm.color, border: `1px solid ${sm.color}40`, borderRadius: 6, padding: '4px 10px', fontSize: 16, fontWeight: 600 }}>
                                        {sm.label}
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                                    {services.map(s => {
                                        ...
                                        return (
                                            <span key={s.category} style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '3px 10px', fontSize: 16, color: 'var(--color-text)' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 16, color: 'var(--color-text-subtle)', marginBottom: 12 }}>
                                ...
                                {order.proof_photo_url && (
                                    <div style={{ background: '#10b98110', border: '1px solid #10b98130', borderRadius: 8, padding: '10px 12px', marginBottom: 10 }}>
                                        <div style={{ fontSize: 16, color: '#059669', fontWeight: 600, marginBottom: 4 }}>✓ Proof of Service</div>
                                        <a href={order.proof_photo_url} target="_blank" rel="noopener noreferrer"
                                            style={{ fontSize: 16, color: '#3b82f6', textDecoration: 'none' }}>View proof photo →</a>
                                        {order.proof_notes && <p style={{ fontSize: 16, color: 'var(--color-text-subtle)', margin: '4px 0 0' }}>{order.proof_notes}</p>}
                                    </div>
                                )}
                                ...
                                {proof.orderId === order.id && (
                                    <div style={{ marginTop: 12, background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 8, padding: 12 }}>
                                        <div style={{ fontSize: 16, color: 'var(--color-text)', fontWeight: 600, marginBottom: 8 }}>Submit Proof of Service</div>
                                        ...
                                        <div style={{ display: 'flex', gap: 8 }}>
                                            <button onClick={() => submitProof(order.id)} style={{
                                                background: '#8b5cf6', color: '#fff', border: 'none',
                                                borderRadius: 6, padding: '7px 14px', fontSize: 16, fontWeight: 600, cursor: 'pointer',
                                            }}>Submit</button>
                                            <button onClick={() => setProof({ url: '', notes: '', orderId: null })} style={{
                                                ...
                                                borderRadius: 6, padding: '7px 14px', fontSize: 16, cursor: 'pointer',
                                            }}>Cancel</button>
                                        </div>
                                        {proofMsg && <p style={{ fontSize: 16, color: 'var(--color-primary)', marginTop: 6 }}>{proofMsg}</p>}
                                    </div>
                                )}
                            </Card>
                        );
