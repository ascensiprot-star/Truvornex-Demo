...
export default function CareBridge() {
    ...
    const inputStyle = {
        ...
        borderRadius: 8, padding: '10px 12px', color: 'var(--color-text)', fontSize: 16,
    };
    const labelStyle = { fontSize: 14, color: 'var(--color-text-subtle)', display: 'block', marginBottom: 4 };
    ...
    return (
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '28px 20px 80px' }}>
            <div style={{ marginBottom: 24 }}>
                ...
                <h1 style={{ fontSize: 30, fontWeight: 700, color: 'var(--color-text)', margin: 0 }}>Family Care Bridge</h1>
            ...
            <div style={{ display: 'flex', gap: 4, background: 'var(--color-card)', border: '1px solid var(--color-border)', borderRadius: 10, padding: 4, marginBottom: 24, width: 'fit-content' }}>
                {[['new-order', 'Send Care'], ['orders', 'My Orders']].map(([k, l]) => (
                    <button key={k} onClick={() => setTab(k)} style={{
                        ...
                        border: 'none', borderRadius: 7, padding: '8px 18px', fontSize: 16, fontWeight: 600, cursor: 'pointer',
                    }}>{l}</button>
                ))}
            </div>
            ...
            {tab === 'new-order' && (
                <Card>
                    <h2 style={{ fontSize: 20, fontWeight: 700, color: 'var(--color-text)', margin: '0 0 20px' }}>New Care Bridge Order</h2>
                    <form onSubmit={submitOrder} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                        ...
                        <div style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 10, padding: 16 }}>
                            <div style={{ fontSize: 16, fontWeight: 600, color: 'var(--color-text)', marginBottom: 12 }}>Amount to Send</div>
                            ...
                            {pkrEquiv > 0 && (
                                <div style={{
                                    ...
                                    <span style={{ fontSize: 16, color: 'var(--color-text-subtle)' }}>
                                        1 {form.currency_sent} = {rates[form.currency_sent]} PKR
                                    </span>
                                    <span style={{ fontSize: 21, fontWeight: 700, color: '#10b981' }}>
                                ...
                                {SERVICE_CATS.map(({ key, icon, label }) => {
                                    ...
                                    return (
                                        <button key={key} type="button" onClick={() => toggleService(key)} style={{
                                            ...
                                            borderRadius: 8, padding: '10px', fontSize: 16, cursor: 'pointer', textAlign: 'left',
                                        }}>
                                            <span style={{ fontSize: 19, display: 'block', marginBottom: 3 }}>{icon}</span>
                                            {label}
                                        </button>
                                    );
                                })}
                        ...
                        <button type="submit" disabled={loading} style={{
                            ...
                            borderRadius: 10, padding: '13px', fontSize: 18, fontWeight: 700, cursor: 'pointer',
                        }}>
                    ...
                    {orders.length === 0 ? (
                        <Card>
                            <div style={{ textAlign: 'center', padding: '30px 0' }}>
                                <div style={{ fontSize: 55, marginBottom: 12 }}>🤝</div>
                                <p style={{ color: 'var(--color-text-subtle)', fontSize: 17 }}>
                                    No orders yet. Send your first Care Bridge to family back home.
                                </p>
                                <button onClick={() => setTab('new-order')} style={{
                                    background: 'var(--color-primary)', color: '#fff', border: 'none',
                                    borderRadius: 8, padding: '10px 20px', fontSize: 16, fontWeight: 600, cursor: 'pointer', marginTop: 12,
                        ...
                        return (
                            <Card key={order.id} style={{ marginBottom: 14 }}>
                                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 10 }}>
                                    <div>
                                        <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--color-text)' }}>{order.recipient_name}</div>
                                        <div style={{ fontSize: 14, color: 'var(--color-text-subtle)' }}>{order.recipient_city}, {order.recipient_country} · {order.recipient_phone}</div>
                                    </div>
                                    <div style={{ background: sm.bg, color: sm.color, border: `1px solid ${sm.color}40`, borderRadius: 6, padding: '4px 10px', fontSize: 13, fontWeight: 600 }}>
                                        {sm.label}
                                    </div>
                                </div>
                                ...
                                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', marginBottom: 10 }}>
                                    {services.map(s => {
                                        ...
                                        return (
                                            <span key={s.category} style={{ background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 6, padding: '3px 10px', fontSize: 14, color: 'var(--color-text)' }}>
                                ...
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 14, color: 'var(--color-text-subtle)', marginBottom: 12 }}>
                                ...
                                {order.proof_photo_url && (
                                    <div style={{ background: '#10b98110', border: '1px solid #10b98130', borderRadius: 8, padding: '10px 12px', marginBottom: 10 }}>
                                        <div style={{ fontSize: 14, color: '#059669', fontWeight: 600, marginBottom: 4 }}>✓ Proof of Service</div>
                                        <a href={order.proof_photo_url} target="_blank" rel="noopener noreferrer"
                                            style={{ fontSize: 14, color: '#3b82f6', textDecoration: 'none' }}>View proof photo →</a>
                                        {order.proof_notes && <p style={{ fontSize: 13, color: 'var(--color-text-subtle)', margin: '4px 0 0' }}>{order.proof_notes}</p>}
                                    </div>
                                )}
                                ...
                                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                    {order.status === 'pending' && (
                                        <button onClick={() => updateStatus(order.id, 'scheduled')} style={{
                                            ...
                                            borderRadius: 6, padding: '5px 12px', fontSize: 14, cursor: 'pointer',
                                ...
                                {proof.orderId === order.id && (
                                    <div style={{ marginTop: 12, background: 'var(--color-bg)', border: '1px solid var(--color-border)', borderRadius: 8, padding: 12 }}>
                                        <div style={{ fontSize: 14, color: 'var(--color-text)', fontWeight: 600, marginBottom: 8 }}>Submit Proof of Service</div>
                                        ...
                                        <div style={{ display: 'flex', gap: 8 }}>
                                            <button onClick={() => submitProof(order.id)} style={{
                                                background: '#8b5cf6', color: '#fff', border: 'none',
                                                borderRadius: 6, padding: '7px 14px', fontSize: 14, fontWeight: 600, cursor: 'pointer',
                                            }}>Submit</button>
                                            <button onClick={() => setProof({ url: '', notes: '', orderId: null })} style={{
                                                ...
                                                borderRadius: 6, padding: '7px 14px', fontSize: 14, cursor: 'pointer',
                                            }}>Cancel</button>
                                        </div>
                                        {proofMsg && <p style={{ fontSize: 14, color: 'var(--color-primary)', marginTop: 6 }}>{proofMsg}</p>}
                                    </div>
                                )}
                            </Card>
                        );
