    ...
    const runAction = async (action) => {
        ...
        try {
            if (!simonConfigured) {
                setResults(r => ({
                    ...r,
                    [action.id]: `**Demo Mode — Simon AI not configured.**\n\nTo enable autonomous admin actions, add \`DEEPSEEK_API_KEY\` to your environment variables.\n\n**Task**: ${action.title}\n\n${action.desc}`,
                }));
                ...
                <div className="rounded-2xl p-4 flex items-start gap-3"
                    ...
                    <div>
                        ...
                        <p className="text-xs mt-0.5" style={{ color: 'var(--color-warning)', opacity: 0.8 }}>
                            Add <code style={{ backgroundColor: 'rgba(245,158,11,0.15)', padding: '1px 5px', borderRadius: 4 }}>DEEPSEEK_API_KEY</code> to your environment variables to activate autonomous AI actions. Actions will run in demo mode until configured.
                        </p>
                    </div>
                </div>
            )}
