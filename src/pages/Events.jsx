...
import { Calendar, MapPin, Ticket, Plus, Search, Users, Camera, Utensils, Palette, Clock, Star, ChevronRight, X, Check, Loader2 } from 'lucide-react';
...
export default function Events() {
    ...
    const loadEvents = async () => {
        setLoading(true);
        const { data } = await supabase.from('events').select('*').order('date', { ascending: true }).limit(60);
    ...
    const createEvent = async () => {
        ...
        try {
            const payload = {
                ...
                organizer_name: form.organizer_name || user.user_metadata?.full_name || user.email?.split('@')[0],
                organizer_email: user.email,
                ticket_price: form.is_free ? 0 : form.ticket_price,
                is_free: form.is_free, total_tickets: form.total_tickets,
                tickets_sold: 0, bundle_services: form.bundle_services,
    ...
    const buyTicket = async (event, qty = 1) => {
        ...
        try {
            ...
            toast.success(`Ticket booked! Code: ${code} 🎟️`);
    ...
    return (
        <div className="space-y-6 pb-8">
            ...
            <div className="flex gap-1 bg-zinc-100 dark:bg-zinc-900 p-1 rounded-2xl w-fit">
                {[['browse','📅 Browse'], ['my-tickets','🎟️ My Tickets'], ['venues','🏛️ Venues']].map(([key, label]) => (
                    ...
                    <div className="flex gap-3 flex-wrap">
                        ...
                        <Select value={catFilter} onValueChange={setCatFilter}>
                            ...
                            <SelectContent>
                                ...
                                {Object.entries(CATEGORY_ICONS).map(([c, icon]) => (
                                    <SelectItem key={c} value={c}>{icon} {c.charAt(0).toUpperCase() + c.slice(1)}</SelectItem>
                                ))}
                            </SelectContent>
                        </Select>
                    </div>
                            ...
                            <div className="flex items-start justify-between gap-4">
                                <div className="flex-1 min-w-0">
                                    ...
                                    <p className="text-xs text-zinc-400 mt-0.5">
                                        Qty: {t.quantity} · Code: <span className="font-mono font-bold">{t.ticket_code}</span>
                                    </p>
                                    <p className="text-xs text-zinc-400">{t.total_amount > 0 ? `$${t.total_amount} paid` : 'Free ticket'}</p>
                                </div>
                                <span className={`text-[10px] font-bold px-2 py-1 rounded-full shrink-0 ${t.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-zinc-100 text-zinc-500'}`}>{t.status}</span>
                            </div>
                            <div className="mt-3 p-3 bg-zinc-50 dark:bg-zinc-900 rounded-xl">
            ...
            {tab === 'venues' && (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                    {[
                        ...
                        { type:'hall',       name:'Heritage Ballroom',     cap:300,  price:'$1,200/day',img:'✨', amenities:['vintage decor','dance floor','catering included'] },
                    ].map((v, i) => (
                    ...
                    <div className="space-y-3 pt-1">
                        ...
                        <div className="grid grid-cols-2 gap-3">
                            <Select value={form.category} onValueChange={v => setForm(p => ({ ...p, category: v }))}>
                                ...
                                <SelectContent>{Object.entries(CATEGORY_ICONS).map(([c, icon]) => <SelectItem key={c} value={c}>{icon} {c}</SelectItem>)}</SelectContent>
                            </Select>
                        ...
                        <Input placeholder="Your name (organizer)" value={form.organizer_name} onChange={e => setForm(p => ({ ...p, organizer_name: e.target.value }))} className="rounded-xl" />
                        <div className="grid grid-cols-2 gap-3">
                        ...
                        <div className="space-y-4 pt-1">
                            <div className="bg-zinc-50 dark:bg-zinc-900 rounded-2xl p-4">
                                ...
                                <p className="text-xs text-zinc-400 mt-1 flex items-center gap-1.5">
                                    <Calendar className="h-3 w-3" />{ticketDialog.date}
                                    {ticketDialog.start_time && ` at ${ticketDialog.start_time}`}
                                </p>
                                <p className="text-xs text-zinc-400 flex items-center gap-1.5 mt-0.5">
                                    <MapPin className="h-3 w-3" />{ticketDialog.venue_name}
                                </p>
                            </div>
                            <div className="flex items-center justify-between">
