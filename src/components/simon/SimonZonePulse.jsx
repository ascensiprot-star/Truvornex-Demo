        ...
        <div style={{ padding: '6px 10px', borderBottom: '1px solid var(--color-border)', flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 7 }}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: '-0.01em', color: 'var(--color-text-muted)', lineHeight: 1.3 }}>
                        Zone {label}
                    ...
                    {zoneHealth.trendingServices?.length > 0 && (
                        <div style={{ fontSize: 15, color: 'var(--color-text-subtle)', marginTop: 1, lineHeight: 1.3 }}>
                            Trending: {zoneHealth.trendingServices.slice(0, 2).join(', ')}
                        </div>
                    )}
                </div>
                <span style={{ fontSize: 13, fontWeight: 800, color: '#7c6fcd', letterSpacing: '0.05em', flexShrink: 0 }}>SIMON</span>
            </div>
        </div>
    );
}
