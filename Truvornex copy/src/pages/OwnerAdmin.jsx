...
export default function OwnerAdmin() {
    ...
    const loadAll = () => {
        ...
        Promise.all([
            ...
            setProviders(p); setBookings(b); setServices(s); setCategories(c);
            setLoading(false);
        });
    };
    ...
    useEffect(() => {
        ...
        loadAll();
        const unsubs = [
                e.type === 'create' ? [e.data, ...prev] : e.type === 'update' ? prev.map(p => p.id === e.id ? e.data : p) : prev.filter(p => p.id !== e.id)
            )),
                e.type === 'create' ? [e.data, ...prev] : e.type === 'update' ? prev.map(b => b.id === e.id ? e.data : b) : prev.filter(b => b.id !== e.id)
            )),
        ];
