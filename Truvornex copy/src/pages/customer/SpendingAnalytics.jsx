...
export default function SpendingAnalytics() {
    ...
    const getAiInsight = async () => {
        setAiLoading(true);
            prompt: `You are Simon AI. Analyze this customer's spending data and give personalized savings advice:
Total spent: $${stats.total}
Total bookings: ${stats.count}
Average booking value: $${stats.avg}
Top categories: ${stats.byCategory.map(c => `${c.name}: $${c.value}`).join(', ')}
Monthly trend: ${stats.byMonth.map(m => `${m.name}: $${m.value}`).join(', ')}
Provide 3 specific, actionable recommendations to reduce costs or optimize spending. Be concise and specific.`,
        });
        setAiInsight(res);
        setAiLoading(false);
    };
