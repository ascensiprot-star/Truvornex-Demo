...
export default function ServiceBundles() {
    ...
    const load = async () => {
        const [me, bunds, cats] = await Promise.all([
        ]);
        setUser(me);
        setBundles(bunds);
        setCategories(cats);
    ...
    const joinBundle = async (bundle) => {
        ...
        setJoining(bundle.id);
            participant_emails: [...(bundle.participant_emails || [bundle.organizer_email]), user.email],
            current_participants: (bundle.current_participants || 1) + 1,
        });
