...
export default function NearbyProviders() {
    ...
    useEffect(() => {
        Promise.all([
        ]).then(([p, c]) => { setProviders(p); setCategories(c); setLoading(false); });

        // Real-time provider updates
            if (event.type === 'create') setProviders(prev => [...prev, event.data]);
            else if (event.type === 'update') setProviders(prev => prev.map(p => p.id === event.id ? event.data : p));
            else if (event.type === 'delete') setProviders(prev => prev.filter(p => p.id !== event.id));
        });
        return () => unsub();
    }, []);
