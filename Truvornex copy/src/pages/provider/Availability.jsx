...
export default function Availability() {
    ...
    useEffect(() => {
            if (provs.length > 0) {
                const p = provs[0];
                setProvider(p);
                if (p.working_hours) setHours(p.working_hours);
                if (p.buffer_time != null) setBufferTime(p.buffer_time);
                if (p.slot_interval != null) setSlotInterval(p.slot_interval);
                if (p.advance_booking_days != null) setAdvanceDays(p.advance_booking_days);
                if (p.auto_confirm != null) setAutoConfirm(p.auto_confirm);
                if (p.cancellation_hours != null) setCancellationHours(p.cancellation_hours);
                setBlackouts(schedules);
            }
            setLoading(false);
    }, []);
