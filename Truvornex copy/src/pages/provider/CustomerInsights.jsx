...
export default function CustomerInsights() {
    ...
    const getAiSummary = async () => {
        setAiLoading(true);
            prompt: `You are Simon AI. Analyze these customer patterns for a service provider and give insights:
Total customers: ${customers.length}
Top spenders: ${customers.slice(0, 3).map(c => `${c.email}: $${c.spent}, ${c.bookings.length} bookings`).join(', ')}
Give 3 specific insights about customer behavior and 2 retention strategies.`,
        });
        setAiSummary(res);
        setAiLoading(false);
    };
                ...
                {customers.slice(0, 15).map(c => (
                    <div key={c.email} className="card-premium p-4 flex items-center gap-4">
                        ...
                        <div className="text-right shrink-0">
                            ...
                            {c.bookings.length > 2 && <span className="text-[10px] bg-zinc-900 text-white px-1.5 py-0.5 rounded-full">VIP</span>}
                        </div>
                    </div>
                ))}
            </div>
